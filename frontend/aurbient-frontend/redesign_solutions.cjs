...
,
  'src/pages/SolutionsEnterpriseInfrastructure.jsx': 
        ...
        <div style={{ marginBottom: "80px", background: "linear-gradient(135deg, #0a141b 0%, var(--color-primary) 100%)", padding: "60px", borderRadius: "24px", border: "1.5px solid rgba(255,255,255,0.1)", boxShadow: "0 20px 40px rgba(0,0,0,0.15)", color: "white" }}>
          <div style={{ textAlign: "center", marginBottom: "50px" }}>
            ...
            <h3 style={{ fontSize: "2.2rem", fontWeight: "700", color: "white" }}>Digital Infrastructure Ecosystem</h3>
            <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "1.1rem", maxWidth: "600px", margin: "16px auto 0" }}>High-availability, secure, and scalable cloud networks for modern enterprises.</p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "30px" }}>
            {[
              ...
              <div key={idx} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", padding: "30px", backdropFilter: "blur(10px)", transition: "transform 0.3s ease, border-color 0.3s ease", cursor: "pointer" }} onMouseOver={e => e.currentTarget.style.borderColor = 'var(--color-accent)'} onMouseOut={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'}>
                <div style={{ width: "60px", height: "60px", borderRadius: "14px", background: "rgba(19,170,179,0.15)", color: "var(--color-accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.6rem", marginBottom: "20px" }}>
                  <i className={"fas " + node.icon}></i>
                </div>
                <h4 style={{ fontSize: "1.3rem", fontWeight: "700", marginBottom: "12px", color: "white" }}>{node.title}</h4>
                <p style={{ color: "rgba(255,255,255,0.6)", fontSize: "0.95rem", lineHeight: "1.6", margin: 0 }}>{node.details}</p>
              </div>
            ))}
          </div>
        </div>
...
files.forEach(file => {
  ...
  // Regex to remove the tasks array and activeTask state
  const tasksRegex = /const tasks = \[\s*\{[\s\S]*?\}\s*\];\s*const \[activeTask, setActiveTask\] = useState\(tasks\[0\]\);/;
  content = content.replace(tasksRegex, '');

  // Regex to remove the Simulator UI
  const simulatorRegex = /\{\/\* INTERACTIVE AGENT PLAYGROUND CONSOLE \*\/\}[\s\S]*?<div style=\{\{ marginBottom: "60px" \}\}>[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/;
  content = content.replace(simulatorRegex, uis[file] || '');

  // For the ones where we removed useState completely from usage, we might get an unused import warning.
  // But React is forgiving, or we can leave it.
